        ...
        <div class="box box-primary">
            ...
            <div class="box-body">
                ...
                <div class="form-group col-lg-3">
                    ...
                    <?php echo $form->passwordField($server, 'password', $server->getHtmlOptions('password')); ?>
                    <?php echo $form->error($server, 'password');?>
                </div>
                <div class="form-group col-lg-3">
                    <?php echo $form->labelEx($server, 'vhost');?>
                    <?php echo $form->textField($server, 'vhost', $server->getHtmlOptions('vhost')); ?>
                    <?php echo $form->error($server, 'vhost');?>
                </div>
                <div class="form-group col-lg-3">
                ...
                <div class="form-group col-lg-3">
                    <?php echo $form->labelEx($server, 'queue');?>
                    <?php echo $form->textField($server, 'queue', $server->getHtmlOptions('queue')); ?>
                    <?php echo $form->error($server, 'queue');?>
                </div>
                <div class="form-group col-lg-3">
                    <?php echo $form->labelEx($server, 'exchange');?>
                    <?php echo $form->textField($server, 'exchange', $server->getHtmlOptions('exchange')); ?>
                    <?php echo $form->error($server, 'exchange');?>
                </div>
                <div class="form-group col-lg-3">
                    <?php echo $form->labelEx($server, 'exchange_type');?>
                    <?php echo $form->textField($server, 'exchange_type', $server->getHtmlOptions('exchange_type')); ?>
                    <?php echo $form->error($server, 'exchange_type');?>
                </div>
                <div class="form-group col-lg-3">
                    <?php echo $form->labelEx($server, 'assigned_ips');?>
                    <?php echo $form->textField($server, 'assigned_ips', $server->getHtmlOptions('assigned_ips')); ?>
                    <?php echo $form->error($server, 'assigned_ips');?>
                </div>
                <div class="clearfix"><!-- --></div>
                ...
                <?php } ?>
                <?php if ($server->canConfirmDelivery) { ?>
                <div class="form-group col-lg-3">
                    <?php echo $form->labelEx($server, 'must_confirm_delivery');?>
                    <?php echo $form->dropDownList($server, 'must_confirm_delivery', $server->getYesNoOptions(), $server->getHtmlOptions('must_confirm_delivery')); ?>
                    <?php echo $form->error($server, 'must_confirm_delivery');?>
                </div>
                <?php } ?>
                <div class="clearfix"><!-- --></div>

                <div class="form-group col-lg-12">
                    <?php echo $form->labelEx($server, 'ip_to_domains');?>
                    <div class="callout callout-info" style="margin-bottom:0px;">
                        <?php echo Yii::t('servers', 'Optionally, a list of ip addresses associated with one or multiple regular expressions for domains, using the JSON format. A clear example looks like:');?><br />
                        <?php echo $server->getAttributePlaceholder('ip_to_domains');?>
                    </div>
                    <?php echo $form->textArea($server, 'ip_to_domains', $server->getHtmlOptions('ip_to_domains')); ?>
                    <?php echo $form->error($server, 'ip_to_domains');?>
                </div>
                <div class="clearfix"><!-- --></div>
                <?php
                /**
