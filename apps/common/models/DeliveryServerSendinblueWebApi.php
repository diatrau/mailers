    ...
    public function sendEmail(array $params = array())
    {
        ...
        try {
            ...
            $sendParams = array(
                ...
                'to'      => array($toEmail => sprintf('=?%s?B?%s?=', strtolower(Yii::app()->charset), base64_encode($toName))),
            ...
            if ($response['code'] != 'success' || empty($response['data']['message-id'])) {
                $message = isset($response['message']) ? $response['message'] : print_r($response, 1);
    ...
    protected function preCheckWebHook()
    {
        ...
        try {
            ...
            if ($response['code'] != 'success') {
                throw new Exception(print_r((array)$response, 1));
            }
