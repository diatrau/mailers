...
class CEmailValidator extends CValidator
{
    ...
    {
        if (!$this->preValidateValue($value))
            return false;
...
";
    ...
    private function encodeIDN($value)
    {
        if (preg_match_all('/^(.*)@(.*)$/',$value,$matches))
        {
            if(function_exists('idn_to_ascii'))
                $value=idn_to_ascii($matches[1][0]).'@'.idn_to_ascii($matches[2][0]);
            else
            {
                require_once(Yii::getPathOfAlias('system.vendors.Net_IDNA2.Net').DIRECTORY_SEPARATOR.'IDNA2.php');
                try {
                    $idna   = new Net_IDNA2();
                    $_value = @$idna->encode($matches[1][0]).'@'.@$idna->encode($matches[2][0]);
                    $value  = $_value;
                } catch (Exception $e) {
    ...
    protected function preValidateValue($email)
    {
        ...
        $strlen         = CommonHelper::functionExists('mb_substr') ? 'mb_strlen' : 'strlen';
        $emailLength    = (int)$strlen($email);

        if ($emailLength > 254 || $atSignPosition === false) {
            return false;
        }

        list($username, $domain) = explode('@', $email);
        $substr = CommonHelper::functionExists('mb_substr') ? 'mb_substr' : 'substr';
        if (in_array($substr($domain, 0, 1), $this->getEmailDomainNotAllowedFirstChar())) {
            return false;
        }
        
        foreach (array('-', '_') as $chr) {
            if (strpos($username, $chr) === 0 || strrpos($username, $chr) === 0) {
                return false;
            }
        }
     ...
     * @return array
     */
    protected function getEmailDomainNotAllowedFirstChar()
    {
        return array(
            '.', '/', '\\', '!', '@', '#', '$', '%', '^',
            '&', '*', '(', ')', '±', '§', '+', '=', '`',
